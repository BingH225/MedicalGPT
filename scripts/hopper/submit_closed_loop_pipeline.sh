...
submission_log="${OUTPUT_ROOT}/closed_loop_pipeline_submission_${submission_stamp}.txt"
...
rm_job_id="$(qsub -v "${QSUB_ENV_VARS}" -W depend=afterok:${sft_job_id} "${SCRIPT_DIR}/run_rm_qwen35_2b.pbs")"
rloo_job_id="$(qsub -v "${QSUB_ENV_VARS}" -W depend=afterok:${sft_job_id}:${rm_job_id} "${SCRIPT_DIR}/run_rloo_qwen35_2b.pbs")"
dpo_job_id="$(qsub -v "${QSUB_ENV_VARS}" -W depend=afterok:${sft_job_id} "${SCRIPT_DIR}/run_dpo_qwen35_2b.pbs")"
